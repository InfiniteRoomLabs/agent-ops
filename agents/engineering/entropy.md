...
- **bw-sync.sh**: Located at `~/projects/infinite-room-labs/infinite-room-labs-infra/scripts/bw-sync.sh`. Supports `--target ansible|k8s|both`, `--check-rotation`, `--verify-k8s`, `--dry-run`, `--quiet`.
