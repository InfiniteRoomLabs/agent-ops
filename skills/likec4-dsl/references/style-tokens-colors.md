...
## Tag colors are not style colors

`tag <name> { color ... }` in `specification` accepts only `#hex`, `rgb()`, or `rgba()`. The semantic tokens above (`gray`, `primary`, ...) are valid in element `style { color ... }` blocks but fail to parse as a tag color (`Expecting: one of these possible Token sequences: rgba, rgb, Hash`). Use `color #9e9e9e`, not `color gray`, on a tag.
