...
description: >
  ...
  persona. Discovers agent definitions across all divisions, parses
...
Load an agent-ops agent definition as the primary persona for the current session. Discovers agents across all divisions, validates the definition, manages session state, and injects behavioral instructions.
...
| `[division] <name>` | Load | Proceed to Phase 2 |

### Division Detection

If two tokens are provided, check whether the first token matches a known division directory name: `core`, `engineering`, `design`, `marketing`, `sales`, `paid-media`, `product`, `project-management`, `testing`, `support`, `research`, `spatial-computing`, `game-development`, `specialized`. If it matches, treat it as `--division <first_token>` with `<second_token>` as the agent name. Otherwise treat the entire input as the agent name.

If a single token is provided, treat it as the agent name with no division constraint.
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/scripts/summon.py discover <name> [--division <div>]
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/scripts/summon.py state create --agent <name> --division <division> --source <path>
```
