...
- **Self-advance is the last ship step, every run:** update "Shipped so far", strike the Retarget row (add merge sha + one-line lesson), append Lessons, then advance to the **next approved phase** -- rewrite the `## Goal` block for it (stage 1 = verify premise; stage 2 = branch + implementer model + work-order template + deliverables with acceptance criteria; stage 3 = gate with reviewer tier one up; stage 4 = ship + retarget). **If no approved phase remains, do not invent one:** park under `## Blocked on user` naming the proposed rows for the user to approve, and stop (see the roadmap-cap rule below).
- **Mid-run replanning is legitimate, but it proposes -- it does not extend the run.** A discovered gap is written as a row under **Proposed (needs user approval)**, never promoted straight into the approved roadmap. Reordering already-approved rows is fine; adding a new phase the loop then runs itself is not. Lessons are append-only and locked decisions stay locked unless the user reverses them.
- **Roadmap cap: the run ends when the APPROVED roadmap is exhausted, not when the backlog empties.** The approved roadmap is the set of rows the user signed off (the spec's roadmap, plus any Proposed row the user later approved). The Done-when clause "retarget to the next phase" is satisfied by parking under `## Blocked on user` when no approved phase is left -- that is a clean goal completion, not a failure. Without this, the loop mines its own backlog forever: every phase's review lanes surface real, useful follow-on work, `retarget to the next phase` always finds a candidate, and the model cannot clear its own goal, so it runs until the user stops it by hand. (freshbooks-tools: an approved 6-phase roadmap self-extended to 11 phases and $407 of extra teammate spend before the user retargeted GOAL.md by hand; every extension was genuinely useful, which is exactly why nothing stopped it.) The five useful extensions were not the bug; the absent stop was.
...
- A treadmill with no roadmap cap does not stop on its own. Review lanes always find more real work, so an uncapped run keeps promoting backlog into live phases and spending until the user notices. Keep discovered work in **Proposed (needs user approval)** and let the run end when the approved roadmap is empty.
