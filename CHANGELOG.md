...
- **Deploy secrets-sync hook** -- aligned the Bitwarden session resolution order (env var -> secret manager -> local cache) with the operator's secrets tooling. (Hook later removed before open-sourcing; it was environment-specific operational glue.)
